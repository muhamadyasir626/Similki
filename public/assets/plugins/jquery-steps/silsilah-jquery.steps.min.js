...
! function (a, b) {
  ...
  function E(b, c, d, e) {
    ...
    if (b.enablePagination) {
      ...
      // Periksa apakah semua langkah telah selesai
      var allStepsCompleted = true;
      a.find(".steps li").each(function() {
        if (!$(this).hasClass("done")) {
          allStepsCompleted = false; // Jika ada langkah yang belum selesai
        }
      });
  
      if (allStepsCompleted) {
        d._enableAria(); // Mengaktifkan tombol finish
        d.removeClass('disabled'); // Menghilangkan kelas disabled
        d.find("a[href$='#finish']").css("background-color", "#28a745"); // Ubah warna tombol (misalnya hijau)
      } else {
        d._disableAria(); // Menonaktifkan tombol finish
        d.addClass('disabled'); // Menambahkan kelas disabled
        d.find("a[href$='#finish']").css("background-color", "#ccc"); // Ubah warna tombol menjadi abu-abu
      }
    }
  }
  ...
  function L(a, b, c) {
    if (b.enablePagination) {
      var d = '<{0} class="actions {1}"><ul role="menu" aria-label="{2}">{3}</ul></{0}>',
        ...
        f = "";
       b.enableFinishButton && (f += e.format("finish", b.labels.finish)), b.enableCancelButton && (f += e.format("cancel", b.labels.cancel)), a.append(d.format(b.actionContainerTag, b.clearFixCssClass, b.labels.pagination, f)), D(a, b, c), A(a, b, c)
    }
  }
