...
<nav class="page-breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item"><a href="{{ route('daftar-pengajuan-lk') }}">Daftar @if(!$statusForm) Pengajuan @endif Lembaga Konservasi</a></li>
    <li class="breadcrumb-item active" aria-current="page">Detail @if(!$statusForm) Pengajuan @endif Lembaga Konservasi</li>
  </ol>
</nav>
  ...
  <div class="row">
    <section>
      <sub-section class="table">
        <div class="header">
          <h4>Detail @if(!$statusForm) Pengajuan @endif Lembaga Konservasi</h4>
          <div class="btn-header">
            ...
            @elseif((Auth::user()->role->tag == 'UPT' && $status->status == 'rejected') || Auth::user()->role->tag == 'LK')
            
            <button class="btn btn-primary" id="{{ $data->id }}" onclick="confirmEdit(this)">Edit</button>
            {{-- <button class="btn btn-primary" id="{{ $data->id }}" name="{{ $status->nama }}" onclick="confirmEdit(this)">Edit</button> --}}

            @if ((Auth::user()->role->tag == 'UPT' || Auth::user()->role->tag == 'KKHSG') && !$status)
        ...
        <div class="table">
          ...
          <table class="table table-bordered">
              <thead>
              <tr>
                  <td>Nama @if(!$statusForm)Calon @endif LK</td>
                  <td>{{ $data->nama }}</td>
              </tr>
              </thead>
              <tbody>
            ...
            <tr>
              <td>Site Plan</td>
              <td>{{$data->doc_site_plan }} - <a href="{{ $data->doc_site_plan }}">Lihat Dokumen</a></td>
            </tr>

            <tr>
              <td>Dokumen Persetujuan Lingkungan</td>
              <td>{{$data->doc_persetujuan_lingkungan }} - <a href="{{ $data->doc_persetujuan_lingkungan }}">Lihat Dokumen</a></td>
            </tr>

            <tr>
              <td>Draft RKP</td>
              <td>{{$data->doc_draft_rkp }} - <a href="{{ $data->doc_draft_rkp }}">Lihat Dokumen</a></td>
            </tr>
